...
instance [Monad m] [inst : ∀ α, Lean.Order.CCPO (m α)] [CCPOBot m] [CCPOBotLawful m] : CCPOBotLawful (StateT σ m) where
  prop := by
    intro α
    ext s
    have hbot : (Lean.Order.bot : StateT σ m α) s = (Lean.Order.bot : m (α × σ)) := by
      rw [show (Lean.Order.bot : StateT σ m α) =
        Lean.Order.CCPO.csup (c := Lean.Order.empty_chain (StateT σ m α)) (by
          intro x y hx
          exact False.elim hx) by rfl]
      rw [← Lean.Order.fun_csup_eq
        (c := Lean.Order.empty_chain (StateT σ m α))
        (hc := by
          intro x y hx
          exact False.elim hx)]
      simp [Lean.Order.fun_csup, Lean.Order.empty_chain, Lean.Order.bot]
    calc
      (CCPOBot.compBot (m := StateT σ m) (α := α)) s
          = CCPOBot.compBot (m := m) (α := α × σ) := rfl
      _ = (Lean.Order.bot : m (α × σ)) := CCPOBotLawful.prop (m := m)
      _ = (Lean.Order.bot : StateT σ m α) s := hbot.symm
...
instance [Monad m] [LawfulMonad m] [_root_.CompleteLattice l] [inst: MAlgOrdered m l]
  ...
  csup_lift {α} chain := by
    ...
    simp [MAlg.lift_StateT, Lean.Order.fun_csup]
...
instance [Monad m] [LawfulMonad m] [_root_.CompleteLattice l] [inst: MAlgOrdered m l]
  ...
  bot_lift := by
    ...
    have hbot : (Lean.Order.bot : StateT σ m α) s = (Lean.Order.bot : m (α × σ)) := by
      ...
      simp [Lean.Order.fun_csup, Lean.Order.empty_chain, Lean.Order.bot]
    calc
      MAlg.lift (Lean.Order.bot : StateT σ m α) post s
          = MAlg.lift ((Lean.Order.bot : StateT σ m α) s) (fun xs => post xs.1 xs.2) := by
              simp [MAlg.lift_StateT]
      _ = MAlg.lift (Lean.Order.bot : m (α × σ)) (fun xs => post xs.1 xs.2) := by rw [hbot]
    exact MAlgTotal.bot_lift (m := m) (post := fun xs : α × σ => post xs.1 xs.2)
